...
try:
    ...
    print('✗ 缺少 python-docx，请运行: pip install python-docx', file=sys.stderr)
    sys.exit(1)
...
def fetch_pubmed_metadata(pmids: list, cache_path: Path = None) -> dict:
    ...
    if cache_path:
        ...
        if cached:
            ...
            if hit:
                print(f'  ℹ  缓存命中 {hit} 篇，剩余 {len(pmids)} 篇待拉取')
...
def generate_ris(metadata: dict, output_path: Path):
    if output_path.exists():
        print(f'  ℹ  RIS 已存在，跳过（可共用）: {output_path}')
        return
...
def open_file(path: Path):
    ...
    try:
        ...
        print(f'  ⚠  无法自动打开 {path.name}: {exc}', file=sys.stderr)
        print(f'     请手动打开: {path}', file=sys.stderr)
...
def main():
    ...
    if styles_dir and not (styles_dir / f'{style_name}.csl').exists():
        print(f'⚠  样式 [{style_name}] 未安装，正在下载...')
        if not install_style(style_name):
            print(f'✗  无法安装样式 {style_name}，请手动安装后重试：')
            print(f'   python search_styles.py --install {style_name}')
            sys.exit(1)
    elif not styles_dir:
        print(f'⚠  未找到 Zotero 样式目录，跳过样式检查')
    ...
    if not pmids:
        print('ℹ  未发现 [PMID: xxxx] 标记，无需处理。')
        sys.exit(0)
    print(f'🔍 发现 {len(pmids)} 个唯一 PMID')
    ...
    if failed_pmids:
        print(f'  ⚠  以下 {len(failed_pmids)} 个 PMID 获取失败: {", ".join(failed_pmids)}')
        print(f'     请检查网络后重试: python process_references.py "<file>" --force-refresh')
    else:
        print(f'✓ 获取完成（{fetched}/{len(pmids)} 成功）')
    print(f'✓ 获取完成（{fetched}/{len(pmids)} 成功）')
    ...
    print('✅ 完成！')
    print()
    print(f'  文档中 {total_refs} 处引用')
    print(f'  处理了 {len(pmids)} 篇文献')
    print(f'  Word 文档: {docx_path}')
    print(f'  RIS 文件:  {ris_path}')
    print('─' * 50)
