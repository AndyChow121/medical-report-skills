...
def search_zotero_styles(query: str) -> list:
    ...
    try:
        ...
        print(f'  ⚠  Zotero API 失败: {e}，尝试 GitHub...')
...
def install_style(style_id: str) -> bool:
    ...
    styles_dir = find_zotero_styles_dir()
    if not styles_dir:
        print('  ✗  未找到 Zotero 样式目录，请手动安装：')
        print(f'     Zotero → Edit → Preferences → Cite → Styles → 搜索 {style_id}')
        return False

    dest = styles_dir / f'{style_id}.csl'
    if dest.exists():
        print(f'  ✓  {style_id} 已安装')
        return True
    ...
    req = urllib.request.Request(url, headers={'User-Agent': 'format-references-zotero/1.0'})
    try:
        print(f'  ↓  下载 {style_id}.csl ...', end=' ', flush=True)
        with urllib.request.urlopen(req, timeout=15) as resp:
            csl = resp.read()
...
def main():
    ...
    styles_dir = find_zotero_styles_dir()
    print(f'找到 {len(results)} 个样式：\n')
    print(f'  {"I"} {"样式 ID":<45} 名称')
    print(f'  {"-"} {"-"*45} {"-"*35}')
    ...
    print()
    print('✓ = 已安装    安装命令: python search_styles.py --install <style-id>')
    print(f'\n使用方式:')
