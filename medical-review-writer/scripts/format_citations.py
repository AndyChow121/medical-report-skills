...
def process_file(file_path: str, inplace: bool = False, output_path: Optional[str] = None):
    ...
    # Detect compound patterns that the regex can't handle
    compound_pattern = re.compile(r'\[PMID:[^\]]*,[^\]]*\]', re.IGNORECASE)
    compound_matches = compound_pattern.findall(content)
    if compound_matches:
        print("="*60, file=sys.stderr)
        print("WARNING: Found compound PMID patterns that will be SKIPPED!", file=sys.stderr)
        print("format_citations.py requires EACH PMID in a separate [PMID: xxx] tag.", file=sys.stderr)
        print("The following patterns will be ignored:", file=sys.stderr)
        for m in compound_matches:
            print(f"  {m}", file=sys.stderr)
        print(file=sys.stderr)
        print("Fix these in the source file before continuing:", file=sys.stderr)
        print("  BAD:  [PMID: xxx, PMID: yyy]", file=sys.stderr)
        print("  GOOD: [PMID: xxx] [PMID: yyy]", file=sys.stderr)
        print("="*60, file=sys.stderr)
    ...
    # Let's do batches of 100 to be safe and fast.
    ...
    if target_path:
        # Post-formatting validation: check for residual [PMID: patterns
        with open(target_path, "r", encoding="utf-8") as f_check:
            final_content = f_check.read()
        residual = re.findall(r'\[PMID:\s*\d+\]', final_content, re.IGNORECASE)
        if residual:
            print("\n" + "="*60, file=sys.stderr)
            print(f"ERROR: {len(residual)} residual [PMID:xxx] tags found in output!", file=sys.stderr)
            print("The regex did not match all PMID tags. Possible causes:", file=sys.stderr)
            print("  1. Compound format [PMID: xxx, PMID: yyy] - already warned above", file=sys.stderr)
            print("  2. Unusual whitespace or characters around PMID", file=sys.stderr)
            print("Fix the source file and re-run. DO NOT edit the output file directly.", file=sys.stderr)
            print("="*60, file=sys.stderr)
...
def main():
    ...
    print("当前状态: 阶段六 (格式化引用) - 引用格式化顺利完成，生成最终排版文档。")
    print("下一步建议: ")
    print("- 整个 Medical Review 工作流已完美竣工！")
    print("- 请将最终合并且格式化后的文档呈现或交付给用户。")
    print("="*40 + "\n")
