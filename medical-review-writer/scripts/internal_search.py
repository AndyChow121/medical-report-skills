...
def main():
    ...
    print("\n" + "="*40, file=sys.stderr)
    print("【状态机 (State Machine)】", file=sys.stderr)
    print("当前状态: 内部库文献检索完成。", file=sys.stderr)
    print("下一步建议: ", file=sys.stderr)
    print("- 请根据检索到的 JSON 结果分析相关信息或辅助当前章节撰写。", file=sys.stderr)
    print("- 也可视情况调整检索条件后重新检索。", file=sys.stderr)
    print("="*40 + "\n", file=sys.stderr)
