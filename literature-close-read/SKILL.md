...
- Python 3.8+
- At least one of: `pymupdf` (`pip install pymupdf`), `pypdf` (`pip install pypdf`), `pdfplumber` (`pip install pdfplumber`)
- The script tries each library in order and uses the first that succeeds; no OCR is performed.

## Example Usage

```bash
# 1) (Optional) Convert PDF to Markdown if you only have a PDF
#    Uses scripts/extract_pdf_text.py — auto-selects pymupdf / pypdf / pdfplumber
python scripts/extract_pdf_text.py paper.pdf
#    Output: paper.extracted.md (same directory as PDF)
#
#    Optional flags:
#      --out path/to/output.md   Custom output path
#      --max-pages N             Limit extraction to first N pages

# 2) Run the close-reading process (manual or via your orchestration tool):
# Input: paper.extracted.md (full text, contains `## Page XX` pagination)
