...
def get_review_target_range(text_body: str) -> tuple[int, int]:
    """
    For Review type, calculate reference count target from 字数:
    ~1 ref per 100 chars, ±20% tolerance.
    Counts all non-whitespace characters as a reasonable 字数 estimate.
    """
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help="Article type for reference count validation (article: ≥30, review: 字数-based ~1/100字 ±20%)",
    )
        ...
        if args.type == "review":
            ...
            if actual < low or actual > high:
                print(
                    ...
                    f" — review (字數 ≈ {low * 125} 字) requires {low}–{high} refs."
                ...
                print(
                    ...
                    f" (字數-based, review requirement met).",
                    file=sys.stderr,
                )
        else:
...
def validate_output(text: str) -> list[tuple[int, str]]:
    ...
    Skips the ## 参考文献 section, which legitimately contains "PMID: NNN" text.
    """
